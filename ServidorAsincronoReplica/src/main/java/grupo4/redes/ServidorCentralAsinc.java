...
public class ServidorCentralAsinc {
    private String ip;
    private String port;
    ...
    public ServidorCentralAsinc(String rutaConfig) {
    ...
    public ServidorCentralAsinc(String ip, String port, String inproc, int maxSalones, int maxLabs, int aulasMoviles) {
        this.ip = ip;
        this.port = port;
        this.inproc = inproc;
    ...
    private void manejarMensajesTrabajadores(Socket backend, Socket frontend, Queue<String> workerQueue) {
        ...
        if ("READY".equals(command)) {
            // Para DEALER no necesitamos la dirección del trabajador específico
            workerQueue.add("worker");
            System.out.println("[BROKER] Nuevo trabajador marcado como listo");
        } else {
    ...
    private void enviarSolicitudATrabajador(String clientAddr, String requestJson, Socket backend, Queue<String> workerQueue) {
        workerQueue.poll(); // Remover un trabajador de la cola
        System.out.println("[BROKER] Enviando solicitud de " + clientAddr + " a trabajador disponible");
